...
const DatasReservas = () => {
  ...
  useEffect(() => {
    if (selectedQuadra) {
      const fetchExistingConfigs = async () => {
        try {
          const response = await fetch (`http://localhost:5000/quadra/horarios/${selectedQuadra}`);
  ...
  const generateTimeSlots = () => {
    const slots = [];
    const interval = timeInterval === '30min' ? 0.5 : timeInterval === '1h' ? 1 : 2;
  ...
  return (
    <div className={styles.container}>
        ...
        Configuração de Horários para Reserva
        </div>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={`${styles.containerDate} ${step >= 1 ? styles.activeStep : ''}`}>
                    ...
                    <ul>
                      {existingConfigs.map(config => (
                        <li key={config.ID_Config}>
                          {formatDateToBR(config.DataInicio)} a {formatDateToBR(config.DataFim)} -
                              {config.Horarios.length}/ horários - {config.intervalo}
                        </li>
                      ))}
                    </ul>
                    </div>
              ...
              <div className={`${styles.stepContainer} ${step >= 2 ? styles.activeStep : ''}`}>
                ...
                {startDate && endDate && (
                  <div className={styles.selectedPeriod}>
                    <p>
                      Período selecionado: de {formatDateToBR(startDate)} até {formatDateToBR(endDate)}
                    </p>
                  </div>
                )}
          ...
          {step >= 3 && (
            ...
            <label className={styles.subTitle}>3. Selecione os Intervalos de Horários</label>
            <p>Período: {formatDateToBR(startDate)} a {formatDateToBR(endDate)}</p>

            <div className={styles.intervalButtons}>
              <button
                type="button"
                className={`${styles.intervalButton} ${timeInterval === '30min' ? styles.activeInterval : ''}`}
              ...
              <button
                type="button"
                className={`${styles.intervalButton} ${timeInterval === '1h' ? styles.activeInterval : ''}`}
              ...
              <button
                type="button"
                className={`${styles.intervalButton} ${timeInterval === '2h' ? styles.activeInterval : ''}`}
            ...
            <div className={styles.timesGrid}>
              {timeSlots.map(slot => (
                <div key={slot} className={styles.timeOption}>
                  <input
                    type="checkbox"
                    id={`slot-${slot}`}
          ...
          {step >= 4 && (
            <div className={`${styles.stepContainer} ${step >= 4 ? styles.activeStep : ''}`}>
              ...
              <p><strong>Quadra:</strong> {
                quadras.find(q => q.ID_Quadra === selectedQuadra)?.NomeQuadra
              }</p>
                <p><strong>Período:</strong> {formatDateToBR(startDate)} a {formatDateToBR(endDate)}</p>
                <p><strong>Intervalo:</strong> {timeInterval === '30min' ? '30 minutos' : timeInterval === '1h' ? '1 hora' : '2 horas'}</p>
              ...
              <ul className={styles.selectedSlotsList}>
               ...
               <p><strong>Total de intervalos:</strong> {selectedTimeSlots.length}</p>
              </div>
              ...
              <div className={styles.priceInput}>
                <label>Valor por intervalo:</label>
                  ...
                  disabled={loading}
                />
                {!price && <small className={styles.errorText}>Digite um valor</small>}
              </div>
